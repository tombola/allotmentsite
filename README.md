...
To use Docker with PostgreSQL as the database update the `DATABASES` section of `allotmentsite/settings.py` to reflect the following:

```python
# allotmentsite/settings.py
DATABASES = {
    "default": {
...
The `INTERNAL_IPS` configuration in `allotmentsite/settings.py` must be also be updated:
